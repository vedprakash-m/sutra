...
route_mappings = [
    ...
    ('test_update_user_role', 'route_params={"resource": "users", "action": "role", "user_id": target_user_id}'),
    ('test.*users.*filter', 'route_params={"resource": "users"}'),
    ('test.*users.*masked', 'route_params={"resource": "users"}'),
    
    # System endpoints
    ('test_get_system_health', 'route_params={"resource": "system", "action": "health"}'),
    ('test_get_system_stats', 'route_params={"resource": "system", "action": "stats"}'),
    ('test_set_maintenance_mode', 'route_params={"resource": "system", "action": "maintenance"}'),
    
    # LLM endpoints
    ('test_get_llm_settings', 'route_params={"resource": "llm", "action": "settings"}'),
    ('test_update_llm_settings', 'route_params={"resource": "llm", "action": "settings"}'),
    
    # Usage endpoints
    ('test_get_usage_stats', 'route_params={"resource": "usage"}'),
    
    # Test data endpoints
    ('test_reset_test_data', 'route_params={"resource": "system", "action": "reset-test-data"}'),
    ('test_seed_test_data', 'route_params={"resource": "system", "action": "seed-test-data"}'),
]
...
body_fixes = [
    (r'(test_update_user_role.*?req = create_auth_request\([^)]*\))', 
     lambda m: m.group(1).replace(')', ', body=new_role_data)') if 'body=' not in m.group(1) else m.group(1)),
    (r'(test_update_llm_settings.*?req = create_auth_request\([^)]*\))', 
     lambda m: m.group(1).replace(')', ', body=new_settings)') if 'body=' not in m.group(1) else m.group(1)),
    (r'(test_set_maintenance_mode.*?req = create_auth_request\([^)]*\))', 
     lambda m: m.group(1).replace(')', ', body=maintenance_data)') if 'body=' not in m.group(1) else m.group(1)),
]
