...
def fix_all_collections_tests():
    ...
    test_methods = re.findall(
        r"(async def test_\w+.*?(?=async def|\Z))", content, re.DOTALL
    )
...
{indent}    {act_comment}"""
    ...
    replacements = [
        (
            r"mock_cosmos_client\.query_items\.return_value = ",
            "mock_database_manager.query_items.return_value = ",
        ),
        (
            r"mock_cosmos_client\.create_item\.return_value = ",
            "mock_database_manager.create_item.return_value = ",
        ),
        (
            r"mock_cosmos_client\.update_item\.return_value = ",
            "mock_database_manager.update_item.return_value = ",
        ),
        (
            r"mock_cosmos_client\.replace_item\.return_value = ",
            "mock_database_manager.replace_item.return_value = ",
        ),
        (
            r"mock_cosmos_client\.delete_item\.return_value = ",
            "mock_database_manager.delete_item.return_value = ",
        ),
        (
            r"mock_cosmos_client\.create_item\.assert_called_once\(\)",
            "mock_database_manager.create_item.assert_called_once()",
        ),
        (
            r"mock_cosmos_client\.update_item\.assert_called_once\(\)",
            "mock_database_manager.update_item.assert_called_once()",
        ),
        (
            r"mock_cosmos_client\.delete_item\.assert_called_once\(\)",
            "mock_database_manager.delete_item.assert_called_once()",
        ),
    ]
