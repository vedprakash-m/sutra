...
def fix_playbooks_tests():
    ...
    content = re.sub(r'req = mock_auth_request', 'req = create_auth_request(method="GET")', content)
    
    # Step 4: Add mock database manager patch pattern like we used in other APIs
    test_methods = re.findall(r'(async def test_\w+.*?)(?=\n    async def|\n\nclass|\Z)', content, re.DOTALL)
        ...
        if 'create_auth_request' in test_method and 'with patch' not in test_method:
            ...
            if method_line_idx >= 0:
                ...
                if req_line_idx >= 0:
                    ...
                    indent = len(lines[req_line_idx]) - len(lines[req_line_idx].lstrip())
                    patch_lines = [
