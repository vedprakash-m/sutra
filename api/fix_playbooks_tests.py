...
def fix_playbooks_tests():
    ...
    for fix in test_fixes:
        ...
        if fix.get('invalid_json'):
            ...
            if url_pattern:
                request_parts.append(f'url="http://localhost/api/playbooks{url_pattern}"')
    ...
    content = re.sub(method_not_allowed_pattern, method_not_allowed_replacement, content, flags=re.DOTALL)
