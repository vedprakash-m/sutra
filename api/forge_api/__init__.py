...
from shared.models.forge_models import (
    ForgeProject, ForgeArtifact, ForgeTemplate, ForgeAnalytics,
    ForgeStage, ProjectStatus, ProjectPriority, ArtifactType,
...
async def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Implementation Playbook Stage endpoints
            ...
            "generate-coding-prompts", "create-development-workflow", 
            "generate-testing-strategy", "create-deployment-guide", 
            "compile-playbook", "validate-context-integration", 
            "optimize-for-agents", "export-playbook", "quality-validation"
        ]:
...
async def create_forge_project(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(
            json.dumps({"error": "Failed to create project", "details": str(e)}),
            status_code=500,
            mimetype="application/json"
        )
...
async def list_forge_projects(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(
            json.dumps({"error": "Failed to list projects", "details": str(e)}),
            status_code=500,
            mimetype="application/json"
        )
...
async def get_forge_project(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not project_id:
            return func.HttpResponse(
                json.dumps({"error": "Project ID required"}),
                status_code=400,
                mimetype="application/json"
            )
        ...
        return func.HttpResponse(
            json.dumps({"error": "Failed to get project", "details": str(e)}),
            status_code=500,
            mimetype="application/json"
        )
...
async def update_forge_project(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not await user_has_project_access(user_info['user_id'], project, required_permission="edit"):
            return func.HttpResponse(
                json.dumps({"error": "Edit access denied"}),
                status_code=403,
                mimetype="application/json"
            )
        ...
        return func.HttpResponse(
            json.dumps({"error": "Failed to update project", "details": str(e)}),
            status_code=500,
            mimetype="application/json"
        )
...
async def advance_project_stage(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(
            json.dumps({"error": "Failed to advance stage", "details": str(e)}),
            status_code=500,
            mimetype="application/json"
        )
...
async def add_project_artifact(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not project_id or not stage:
            return func.HttpResponse(
                json.dumps({"error": "Project ID and stage required"}),
                status_code=400,
                mimetype="application/json"
            )
        ...
        return func.HttpResponse(
            json.dumps({"error": "Failed to add artifact", "details": str(e)}),
            status_code=500,
            mimetype="application/json"
        )
...
async def ai_enhance_project(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        body = json.loads(req.get_body().decode('utf-8'))
        project_id = body.get('project_id')
        enhancement_type = body.get('enhancement_type')  # "idea_improvement", "validation_criteria", "planning_suggestions", etc.
        context = body.get('context', '')
        
        if not project_id or not enhancement_type:
            return func.HttpResponse(
                json.dumps({"error": "Project ID and enhancement type required"}),
                status_code=400,
                mimetype="application/json"
            )
