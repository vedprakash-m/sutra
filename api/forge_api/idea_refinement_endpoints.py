...
async def analyze_idea(req: func.HttpRequest, project_id: str) -> func.HttpResponse:
    ...
    try:
        ...
        quality_result = quality_engine.calculate_quality_score(
            stage="idea_refinement", content=idea_data, context=project_context
        )
        ...
        return func.HttpResponse(json.dumps(analysis_result), status_code=200, headers={"Content-Type": "application/json"})
...
async def refine_idea_with_llm(req: func.HttpRequest, project_id: str) -> func.HttpResponse:
    ...
    try:
        ...
        await cost_tracker.track_llm_call_start(
            user_id=user_info.get("user_id"), operation="forge_idea_refinement", model=selected_llm, project_id=project_id
        )
        ...
        post_refinement_quality = quality_engine.calculate_quality_score(
            stage="idea_refinement", content=refined_content, context=project_context
        )
        ...
        return func.HttpResponse(json.dumps(refinement_result), status_code=200, headers={"Content-Type": "application/json"})
...
async def complete_stage(req: func.HttpRequest, project_id: str) -> func.HttpResponse:
    ...
    try:
        ...
        quality_result = quality_engine.calculate_quality_score(
            stage="idea_refinement", content=final_idea_data, context=project_context
        )
        ...
        can_complete = quality_result.quality_gate_status in ["PROCEED_WITH_CAUTION", "PROCEED_EXCELLENT"] or force_complete
...
def _generate_next_steps(quality_result: Any, thresholds: Any) -> List[Dict[str, Any]]:
    ...
    if quality_result.quality_gate_status == "BLOCK":
        ...
        next_steps.append(
            {"action": "proceed_to_prd", "priority": "high", "message": "Excellent quality! Ready for PRD generation"}
        )
...
def _create_refinement_prompt(
    current_idea: Dict[str, Any], improvement_focus: List[str], project_context: Dict[str, Any]
) -> str:
...
def _parse_llm_refinement_response(response_content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "marketAnalysis": {"marketSize": "Unknown", "competitors": [], "competitiveAdvantage": "Please analyze manually"},
...
def _prepare_context_for_prd(idea_data: Dict[str, Any], quality_result: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "qualityFoundation": {
            "overallScore": quality_result.overall_score,
            "strongDimensions": [dim for dim, score in quality_result.dimension_scores.items() if score > 85],
            "improvementAreas": [dim for dim, score in quality_result.dimension_scores.items() if score < 75],
        },
    }
