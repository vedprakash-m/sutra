...
def evaluate_architecture_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not user_id or not session_id:
            return func.HttpResponse(
                json.dumps({"error": "Missing user_id or session_id"}),
                status_code=400,
                mimetype="application/json"
            )
        ...
        context_validation = cross_stage_validator.validate_stage_context(
            'technical_analysis', project_context
        )
        ...
        result_document = {
            ...
            "metadata": {
                ...
                "processing_time": sum(r.processing_time for r in technical_evaluation.consensus_metadata.individual_responses),
...
def generate_technical_specs_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        operation_id = cost_tracker.start_operation(
            user_id=user_id,
            operation_type="technical_specs_generation",
...
def assess_feasibility_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not user_id or not project_requirements:
            return func.HttpResponse(
                json.dumps({"error": "Missing project requirements"}),
                status_code=400,
                mimetype="application/json"
            )
        ...
        operation_id = cost_tracker.start_operation(
            user_id=user_id,
            operation_type="feasibility_assessment",
...
def analyze_risks_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not user_id or not technical_context:
            return func.HttpResponse(
                json.dumps({"error": "Missing technical context"}),
                status_code=400,
                mimetype="application/json"
            )
        ...
        operation_id = cost_tracker.start_operation(
            ...
            metadata={"risk_categories": risk_categories}
        )
...
def get_consensus_models_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(
            json.dumps(available_models, default=str),
            status_code=200,
            mimetype="application/json"
        )
...
def generate_implementation_roadmap_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not user_id or not technical_analysis:
            return func.HttpResponse(
                json.dumps({"error": "Missing technical analysis data"}),
                status_code=400,
                mimetype="application/json"
            )
        ...
        operation_id = cost_tracker.start_operation(
            user_id=user_id,
            operation_type="implementation_roadmap",
            metadata={"constraints": project_constraints}
        )
...
def export_technical_analysis_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        include_sections = request_data.get('include_sections', [
            'architecture', 'technology_stack', 'feasibility', 'risks', 'roadmap'
        ])
        ...
        if not items:
            return func.HttpResponse(
                json.dumps({"error": "No technical analysis found for session"}),
                status_code=404,
                mimetype="application/json"
            )
...
def quality_check_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not user_id or not session_id:
            return func.HttpResponse(
                json.dumps({"error": "Missing user_id or session_id parameters"}),
                status_code=400,
                mimetype="application/json"
            )
        ...
        if not items:
            return func.HttpResponse(
                json.dumps({"error": "No session data found"}),
                status_code=404,
                mimetype="application/json"
            )
        ...
        return func.HttpResponse(
            json.dumps(quality_check, default=str),
            status_code=200,
            mimetype="application/json"
        )
