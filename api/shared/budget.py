...
class EnhancedBudgetManager:
    ...
    def _assess_quality_impact(self, current_model: str, alternative_model: str, complexity: str) -> str:
        ...
        model_tiers = {
            "gpt-4": 5, "gpt-4-turbo": 5, "claude-3-opus": 5,
            "claude-3-sonnet": 4, "gemini-1.5-pro": 4,
            "gpt-3.5-turbo": 3, "claude-3-haiku": 3,
            "gemini-pro": 2
        }
...
class BudgetManager(EnhancedBudgetManager):
    ...
    async def track_usage(self, user_id: str, provider: LLMProvider, operation: str,
                         cost: float, tokens_used: int, execution_time_ms: int,
    ...
    async def get_user_usage(self, user_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"user_id": user_id, "total_cost": 0, "total_tokens": 0, "total_requests": 0}
    ...
    async def get_system_usage(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            for record in records:
                ...
                user_usage[user_id]["cost"] += record.get("cost", 0)
                user_usage[user_id]["tokens"] += record.get("tokens_used", 0)
                user_usage[user_id]["requests"] += 1

            # Sort top users by cost
            top_users = [
                {"user_id": uid, "cost": data["cost"], "tokens": data["tokens"], "requests": data["requests"]}
            ...
            return {"total_cost": 0, "total_tokens": 0, "total_requests": 0, "unique_users": 0, "top_users": []}
    ...
    async def get_budget_alerts(self) -> List[Dict[str, Any]]:
        ...
        try:
            query = """
                SELECT * FROM c
                WHERE c.alert_level IN ('warning', 'critical', 'exceeded')
                AND c.is_active = true
                ORDER BY c.created_at DESC
            """

            alerts = await self.db_manager.query_items(
                container_name="budget_alerts",
                query=query
            )
    ...
    async def get_daily_usage(self, user_id: Optional[str] = None, date: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"date": date or "today", "total_cost": 0, "total_tokens": 0, "total_requests": 0}
    ...
    async def estimate_operation_cost(self, provider: LLMProvider, model: str, operation: str,
        ...
        return await self.estimate_execution_cost(model, f"{'x' * input_tokens}", expected_output_tokens)
    ...
    async def start_operation_tracking(self, operation_id: str, user_id: str, provider: LLMProvider, model: str) -> Dict[str, Any]:
        ...
        await self.db_manager.create_item(
            container_name="operation_tracking",
            item=tracking_data
        )
...
def get_enhanced_budget_manager() -> EnhancedBudgetManager:
