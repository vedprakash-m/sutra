...
class BudgetManager:
    ...
    async def get_user_usage(self, user_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            start_date = end_date.replace(day=1) if days >= 30 else end_date.replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    async def get_system_usage(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            for provider in provider_stats:
                provider_stats[provider]['unique_users'] = len(provider_stats[provider]['unique_users'])
            ...
            return {
                ...
                'provider_breakdown': provider_stats,
                'top_users': [{'user_id': user_id, 'cost': cost} for user_id, cost in top_users],
                'daily_usage': await self._get_daily_usage(usage_records)
            }
            ...
            for provider in LLMProvider:
                ...
                if provider_budget['utilization_percent'] > 90:
                    alerts.append({
                        ...
                        'provider': provider,
                        'utilization_percent': provider_budget['utilization_percent'],
    ...
    async def _get_daily_usage(self, usage_records: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
