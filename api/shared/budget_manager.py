...
class BudgetManager:
    ...
    async def _store_budget_limit(self, budget_limit: BudgetLimit) -> None:
        ...
        try:
            container = self.cosmos_client.get_database_client(self.database_name) \
                                         .get_container_client(self.budget_limits_container)
            ...
            budget_dict["actions"] = {str(k): v.value if isinstance(v, BudgetAction) else v 
                                    for k, v in budget_limit.actions.items()}
    ...
    async def get_current_budget_usage(self, budget_id: str, user_id: str) -> Optional[BudgetUsage]:
        ...
        try:
            ...
            # Calculate forecast
            forecast_spend = await self._calculate_forecast_spend(
                budget, total_spent, period_start, period_end
            )
    ...
    async def _store_admin_override(self, override: AdminOverride) -> None:
        ...
        try:
            container = self.cosmos_client.get_database_client(self.database_name) \
                                         .get_container_client(self.admin_overrides_container)
    ...
    async def check_admin_override(self, budget_id: str, user_id: str) -> Optional[AdminOverride]:
        ...
        try:
            ...
            """
            parameters = [
                {"name": "@budget_id", "value": budget_id},
