...
class CostTracker:
    ...
    async def track_llm_usage(
        ...
        try:
            ...
            self.logger.info(
                ...
                f"${float(total_cost):.6f} ({total_tokens} tokens)"
            )
    ...
    async def _store_cost_entry(self, cost_entry: CostEntry) -> None:
        ...
        try:
            container = self.cosmos_client.get_database_client(self.database_name) \
                                         .get_container_client(self.cost_entries_container)
    ...
    def _calculate_summary(
        ...
        for entry in entries:
            ...
            cost_by_provider[provider] = cost_by_provider.get(provider, Decimal("0")) + \
                                       Decimal(str(entry["total_cost"]))
        
        # Group by model
        cost_by_model = {}
        for entry in entries:
            model = f"{entry['provider']}/{entry['model']}"
            cost_by_model[model] = cost_by_model.get(model, Decimal("0")) + \
                                 Decimal(str(entry["total_cost"]))
    ...
    async def _alert_exists(
        ...
        try:
            container = self.cosmos_client.get_database_client(self.database_name) \
                                         .get_container_client(self.cost_alerts_container)
    ...
    async def update_pricing_model(
        ...
        try:
            ...
            self.pricing_models[provider][model] = {
                "input": input_price,
                "output": output_price
            }
        ...
        for model, metrics in efficiency.items():
            if metrics["cost_percentage"] > 50:
                recommendations.append(
                    f"Consider optimizing usage of {model} - accounts for "
                    f"{metrics['cost_percentage']:.1f}% of total costs"
                )
