...
class CostTrackingMiddleware:
    ...
    def track_llm_execution(self, user_id: str, session_id: Optional[str] = None):
        """
        ...
        def decorator(func: Callable[..., Any]):
            ...
            async def wrapper(*args: Any, **kwargs: Any) -> Any:
                ...
                try:
                    ...
                    self.logger.info(f"Starting LLM execution tracking: {request_id} " f"({provider_name}/{model_name})")
                    ...
                    if isinstance(result, LLMResponse) and result.token_usage:
                        ...
                        self.logger.warning(f"No token usage data available for request {request_id}")
    ...
    async def _track_execution_failure(
        ...
        try:
            ...
            self.logger.warning(f"Tracked failed execution: {request_id} - {error}")

        except Exception as e:
            self.logger.error(f"Error tracking execution failure: {str(e)}")

    async def validate_budget_before_execution(
        self, user_id: str, provider: str, model: str, estimated_tokens: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            summary = await self.cost_tracker.get_cost_summary(user_id=user_id, start_date=start_of_month)
    ...
    async def get_real_time_usage_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "analytics": analytics,
                "period": {"start": start_of_month.isoformat(), "end": datetime.now(timezone.utc).isoformat()},
            }
