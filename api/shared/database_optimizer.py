...
class DatabaseOptimizer:
    ...
    def get_optimization_suggestions(self) -> List[str]:
        ...
        if high_ru_queries:
            suggestions.append(
                f"Found {len(high_ru_queries)} high RU cost queries. Consider query optimization or result limiting."
            )
    ...
    async def query_items_optimized(
        ...
        try:
            result = await self.base_manager.query_items(
                container_name=container_name, query=optimized_query, parameters=parameters, partition_key=partition_key
            )

            # Cache the result if appropriate
            if enable_caching and self._should_cache_query(query, container_name):
                self._query_cache[cache_key] = {"data": result, "timestamp": time.time()}
