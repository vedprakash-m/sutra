...
from .validation import ValidationException, SecurityValidationException, BusinessLogicException, RateLimitException
...
class ErrorHandler:
    ...
    def handle_validation_error(
        ...
        if isinstance(exc, ValidationException):
            ...
            main_error = errors[0] if errors else ErrorDetail(
            ...
            logger.info(
                f"Input validation failed: {str(exc)}",
                extra={'request_id': request_id}
            )
...
class ErrorRecovery:
    ...
    @staticmethod
    def get_fallback_response(error_category: ErrorCategory) -> Optional[Dict[str, Any]]:
