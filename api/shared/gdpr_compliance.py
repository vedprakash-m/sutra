...
class GDPRCompliance:
    ...
    async def _process_erasure_request(self, request: DataSubjectRequest) -> None:
        ...
        request.notes = json.dumps({"deletion_summary": deletion_results, "deletion_timestamp": datetime.utcnow().isoformat()})
    ...
    async def _delete_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        for container_name, user_field in deletion_order:
            try:
                    ...
                    for item in items:
                        try:
                            ...
                            results["errors"].append(
                                {"container": container_name, "item_id": item.get("id"), "error": str(delete_error)}
                            )
                ...
                results["errors"].append({"container": container_name, "error": str(e)})
    ...
    async def _process_rectification_request(self, request: DataSubjectRequest) -> None:
        ...
        request.notes = json.dumps({"rectification_applied": True, "timestamp": datetime.utcnow().isoformat()})
    ...
    async def _process_restriction_request(self, request: DataSubjectRequest) -> None:
        ...
        request.notes = json.dumps({"restriction_applied": True, "timestamp": datetime.utcnow().isoformat()})
    ...
    async def _process_objection_request(self, request: DataSubjectRequest) -> None:
        ...
        request.notes = json.dumps({"objection_recorded": True, "timestamp": datetime.utcnow().isoformat()})
                    ...
                    if container:
                        ...
                        for item in items:
                            try:
                                ...
                                if retention_date < now:
                                    container.delete_item(item=item["id"], partition_key=item.get("user_id", item["id"]))
            ...
            if container:
                ...
                if items:
                    ...
                    return ConsentRecord(
                        ...
                        withdrawal_date=(
                            datetime.fromisoformat(item["withdrawal_date"]) if item.get("withdrawal_date") else None
                        ),
        ...
        try:
            ...
            if container:
                if purpose:
                    ...
                    parameters = [{"name": "@userId", "value": user_id}, {"name": "@purpose", "value": purpose.value}]
                else:
