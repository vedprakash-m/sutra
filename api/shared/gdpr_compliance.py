...
class GDPRCompliance:
    ...
    async def record_consent(self, user_id: str, purpose: DataProcessingPurpose,
                           data_categories: List[DataCategory], 
    ...
    async def withdraw_consent(self, user_id: str, consent_id: str = None,
                             purpose: DataProcessingPurpose = None) -> bool:
    ...
    async def check_consent(self, user_id: str, purpose: DataProcessingPurpose,
    ...
    async def record_data_processing(self, user_id: str, purpose: DataProcessingPurpose,
                                   data_categories: List[DataCategory],
                                   lawful_basis: str, consent_id: str = None,
    ...
    async def handle_data_subject_request(self, user_id: str, request_type: DataSubjectRight,
        ...
        try:
            ...
            overview = {
                ...
                "data_categories_processed": list(set([
                    cat.value for record in processing_records 
                    for cat in record.data_categories
                ])),
                "processing_purposes": list(set([
                    record.purpose.value for record in processing_records
                ])),
    ...
    async def _calculate_retention_status(self, user_id: str, 
                                        processing_records: List[DataProcessingRecord]) -> Dict[str, Any]:
        """Calculate data retention status"""
        now = datetime.utcnow()
        retention_status = {
            "items_due_for_deletion": 0,
            "upcoming_deletions": [],
            "permanent_retention_items": 0
        }
    ...
    async def _store_consent(self, consent: ConsentRecord) -> None:
        ...
        if self.database_manager:
            ...
            await self.database_manager.create_item(
                container_name=self.consent_container,
                item=data
            )
    ...
    async def _store_processing_record(self, record: DataProcessingRecord) -> None:
        ...
        if self.database_manager:
            ...
            await self.database_manager.create_item(
                container_name=self.processing_container,
                item=data
            )
    ...
    async def _store_subject_request(self, request: DataSubjectRequest) -> None:
        ...
        if self.database_manager:
            ...
            await self.database_manager.create_item(
                container_name=self.requests_container,
                item=data
            )
    ...
    async def _get_user_consents(self, user_id: str, 
                               purpose: DataProcessingPurpose = None) -> List[ConsentRecord]:
...
def gdpr_required(purpose: DataProcessingPurpose, 
                 data_categories: List[DataCategory],
            ...
            if user_id:
                ...
                if purpose == DataProcessingPurpose.CONSENT:
                    for category in data_categories:
                        has_consent = await gdpr_compliance.check_consent(
                            user_id, purpose, category
                        )
                        if not has_consent:
