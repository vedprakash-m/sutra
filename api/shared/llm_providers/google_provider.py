...
class GoogleProvider(BaseLLMProvider):
    ...
    async def initialize(self, kv_client) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Google AI configured with API key ending in: ...{self.api_key[-4:] if self.api_key else 'None'}")
            return True
        ...
        try:
            ...
            if stream:
                return self._handle_streaming_response(
                    model_instance, prompt, generation_config, safety_settings, model
                )
            else:
