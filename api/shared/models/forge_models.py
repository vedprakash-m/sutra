...
class ForgeProject:
    ...
    def get_current_stage_data(self) -> Union[ConceptionData, ValidationData, PlanningData, ImplementationData, DeploymentData]:
                ...
                for milestone_dict in planning_dict['milestones']:
                    if isinstance(milestone_dict, dict):
                        ...
                        if 'due_date' in milestone_dict and isinstance(milestone_dict['due_date'], str):
                            milestone_dict['due_date'] = datetime.fromisoformat(milestone_dict['due_date'].replace('Z', '+00:00'))
                        if 'actual_completion_date' in milestone_dict and isinstance(milestone_dict['actual_completion_date'], str):
                            milestone_dict['actual_completion_date'] = datetime.fromisoformat(milestone_dict['actual_completion_date'].replace('Z', '+00:00'))
                        # Convert enum strings back to enums
                    ...
                    if isinstance(artifact_dict, dict):
                        ...
                        for date_field in ['created_at', 'updated_at']:
                            if date_field in artifact_dict and isinstance(artifact_dict[date_field], str):
                                try:
                                    artifact_dict[date_field] = datetime.fromisoformat(artifact_dict[date_field].replace('Z', '+00:00'))
                                except ValueError:
                                    pass
                        # Convert enum strings back to enums
...
def calculate_stage_completion_percentage(project: ForgeProject, stage: ForgeStage) -> float:
    ...
    completed_fields = sum(1 for value in asdict(stage_data).values() 
