...
class CrossStageQualityValidator:
    ...
    def __init__(self):
        ...
        self.stage_dependencies = {
            "prd_generation": {
                ...
                "context_keys": [
                    "problemStatement", "targetAudience", "valueProposition", 
                    "marketContext", "qualityMetrics"
                ],
            ...
            "ux_requirements": {
                ...
                "context_keys": [
                    "problemStatement", "targetAudience", "userStories",
                    "functionalRequirements", "qualityMetrics"
                ],
            ...
            "technical_analysis": {
                ...
                "context_keys": [
                    "problemStatement", "functionalRequirements", "userJourneys",
                    "designSpecs", "qualityMetrics"
                ],
            ...
            "implementation_playbook": {
                ...
                "context_keys": [
                    "problemStatement", "technicalArchitecture", "designSpecs",
                    "implementationSpecs", "qualityMetrics"
                ],
    ...
    def validate_stage_readiness(
        ...
        for required_stage in stage_config["requires"]:
            ...
            if stage_quality < required_quality:
                ...
                recommendations.append(
                    f"Improve {required_stage} quality before proceeding to {target_stage}"
                )
        ...
        consistency_score = self._calculate_stage_readiness_score(
            target_stage, project_data, validation_errors, context_gaps
        )
        ...
        for rule_name, rule_config in rules.items():
            ...
            if not source_value or not target_value:
                validation_warnings.append(
                    f"Missing data for consistency check: {rule_name}"
                )
                continue
    ...
    def prepare_context_handoff(
        ...
        # Calculate consistency score
        consistency_validation = self.validate_cross_stage_consistency(
            source_stage, target_stage, project_data
        )
    ...
    def validate_quality_progression(
        self,
        project_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if quality_progression:
            ...
            quality_trend = "increasing" if len(quality_progression) < 2 else (
                "increasing" if quality_progression[-1]["quality_score"] > quality_progression[0]["quality_score"]
                else "stable" if abs(quality_progression[-1]["quality_score"] - quality_progression[0]["quality_score"]) < 3
                else "decreasing"
            )
        else:
            avg_quality = 0
    ...
    def _generate_progression_recommendations(
        ...
        if below_threshold:
            recommendations.append(
                f"Improve quality in {len(below_threshold)} stage(s) below threshold"
            )
        ...
        if len(quality_progression) >= 2:
            ...
            if latest_quality < previous_quality:
                recommendations.append(
                    "Quality is declining - review recent stage implementation for improvements"
                )
            elif latest_quality > previous_quality + 10:
                recommendations.append(
                    "Excellent quality improvement trend - maintain current practices"
                )
