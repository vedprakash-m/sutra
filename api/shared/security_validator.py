...
class InputValidator:
    ...
    def __init__(self, config: SecurityConfig = None):
        ...
        self.patterns = {
            ...
            'sql_injection': re.compile(r'(\b(SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|EXEC|EXECUTE|UNION|SCRIPT)\b|--|\'|"|;|\*|%|\\)', re.IGNORECASE),
            'xss_patterns': re.compile(r'(<script|<iframe|<object|<embed|javascript:|vbscript:|onload=|onerror=|onclick=)', re.IGNORECASE)
        }
    ...
    def validate_string(self, value: str, min_length: int = 0, max_length: int = None, 
                       pattern: str = None, required: bool = False) -> str:
    ...
    def validate_integer(self, value: Any, min_val: int = None, max_val: int = None, 
    ...
    def validate_float(self, value: Any, min_val: float = None, max_val: float = None, 
    ...
    def validate_list(self, value: Any, item_validator: Callable = None, 
                     min_items: int = 0, max_items: int = None, required: bool = False) -> List[Any]:
        """Validate list input"""
        if value is None:
            if required:
    ...
    def validate_dict(self, value: Any, schema: Dict[str, ValidationRule] = None, 
            ...
            try:
                if field_value is None and rule.required:
                    raise ValidationError(f"Field '{field_name}' is required", 
                                        field=field_name, code="REQUIRED")
...
def validate_request(schema: Dict[str, ValidationRule] = None, 
                    csrf_required: bool = False, rate_limit: bool = True):
...
PROMPT_SCHEMA = {
    ...
    'tags': ValidationRule(
        ...
        required=False,
        sanitizer=lambda x: validator.validate_list(x, item_validator=lambda tag: validator.validate_string(tag, max_length=50))
    ),
