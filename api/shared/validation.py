...
from .models import (
    UserRole, PromptStatus, PlaybookStatus, LLMProvider,
    User, PromptTemplate, PromptVariable, Collection, 
    Playbook, PlaybookStep, LLMProviderConfig, UsageRecord
)
...
def validate_tags(tags: List[str]) -> List[str]:
    ...
    for tag in tags:
        ...
        if not re.match(r'^[a-zA-Z0-9\-_]+$', tag):
            raise ValidationException("Tags can only contain letters, numbers, hyphens, and underscores", "tags")
...
class PromptTemplateValidator:
    ...
    def validate_update_request(data: Dict[str, Any], existing_prompt: PromptTemplate) -> Dict[str, Any]:
        ...
        updatable_fields = ['title', 'description', 'content', 'variables', 'tags', 'status']
            ...
            if field in data:
                if field == 'title':
                    ...
                    for i, var in enumerate(variables):
                        ...
                        if var_type not in ['string', 'number', 'boolean']:
                            raise ValidationException(f"Invalid variable type: {var_type}", f'{field}[{i}].type')
