...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        debug_info = {
            ...
            "auth_header_type": auth_header.split()[0]
            if auth_header and " " in auth_header
            else None,
            "token_length": len(auth_header.split()[1])
            if auth_header and " " in auth_header
            else 0,
            "principal_header": bool(headers.get("x-ms-client-principal")),
            "environment_vars": {
                "SUTRA_ENTRA_ID_CLIENT_ID": os.getenv(
                    "SUTRA_ENTRA_ID_CLIENT_ID", "NOT_SET"
                )[:50]
                + "..."
                if os.getenv("SUTRA_ENTRA_ID_CLIENT_ID")
                and len(os.getenv("SUTRA_ENTRA_ID_CLIENT_ID", "")) > 50
                else os.getenv("SUTRA_ENTRA_ID_CLIENT_ID", "NOT_SET"),
                "SUTRA_ENTRA_ID_AUTHORITY": os.getenv(
                    "SUTRA_ENTRA_ID_AUTHORITY", "NOT_SET"
                ),
