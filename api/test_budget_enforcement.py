...
from shared.budget_manager import (
    BudgetManager, BudgetPeriod, BudgetAction, BudgetStatus,
    BudgetLimit, BudgetUsage, AdminOverride
)
...
class BudgetEnforcementTest:
    ...
    async def test_budget_enforcement_scenarios(self):
        ...
        for i, scenario in enumerate(test_scenarios, 1):
            ...
            for action in enforcement_result['enforcement_actions']:
                print(f"      Action: {action['action']} at {action['threshold']}% "
                      f"(${action['projected_spend']:.2f}/${action['budget_limit']:.2f})")
            
            for warning in enforcement_result['warnings']:
                print(f"      Warning: {warning.get('action', 'Unknown')} at "
                      f"{warning.get('threshold', 0)}%")
    ...
    async def test_admin_overrides(self):
        ...
        active_override = await self.budget_manager.check_admin_override(
            test_budget.id, self.test_user_id
        )
    ...
    async def test_budget_usage_tracking(self):
        ...
        # Get current usage
        usage = await self.budget_manager.get_current_budget_usage(
            test_budget.id, self.test_user_id
        )
    ...
    async def test_budget_reporting(self):
        ...
        report = await self.budget_manager.get_budget_report(
            user_id=self.test_user_id,
            period_days=30
        )
        ...
        for budget in report.get("budgets", []):
            print(f"      - {budget['name']}: ${budget['spent']:.2f}/${budget['amount']:.2f} "
        ...
        for i, budget in enumerate(budgets, 1):
            ...
            if usage:
                ...
                if usage.days_remaining > 0:
                    daily_rate = usage.total_spent / max(1, (
                        datetime.now(timezone.utc) - usage.period_start
                    ).days or 1)
                    projected_spend = daily_rate * (
                        (usage.period_end - usage.period_start).days
                    )
