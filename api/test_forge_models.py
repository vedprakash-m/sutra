...
from shared.models.forge_models import (
    ForgeProject, ForgeArtifact, ConceptionData, ValidationData, PlanningData,
    ImplementationData, DeploymentData, ForgeTemplate, ForgeAnalytics,
    ForgeStage, ProjectStatus, ProjectPriority, ArtifactType, ValidationStatus,
    TaskStatus, DeploymentStatus, ValidationCriteria, ProjectResource,
    ProjectMilestone, ImplementationTask, DeploymentEnvironment,
    generate_forge_id, validate_stage_transition, calculate_stage_completion_percentage
)
...
class ForgeModelsTest:
    ...
    async def test_stage_transitions(self):
        ...
        stages = [ForgeStage.CONCEPTION, ForgeStage.VALIDATION, ForgeStage.PLANNING, 
                 ForgeStage.IMPLEMENTATION, ForgeStage.DEPLOYMENT]
        ...
        for i, expected_stage in enumerate(stages):
            ...
            if i > 0:
                ...
                assert previous_stage.value in project.stage_completed_at, f"Previous stage {previous_stage.value} should be marked completed"
            ...
            for j, target_stage in enumerate(stages):
                ...
                assert actual_valid == expected_valid, f"Transition validation failed for {project.current_stage.value} -> {target_stage.value}"
    ...
    async def test_artifact_management(self):
        ...
        artifact_types = [ArtifactType.DIAGRAM, ArtifactType.CODE, ArtifactType.RESEARCH, 
                         ArtifactType.ANALYSIS, ArtifactType.SPECIFICATION]
    ...
    async def test_stage_specific_data(self):
        ...
        print(f"  ✅ Planning: {len(project.planning_data.resource_requirements)} resources, {len(project.planning_data.milestones)} milestones")
