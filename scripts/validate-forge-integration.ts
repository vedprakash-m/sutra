...
if (fileExists(typeFile)) {
  ...
  requiredTypes.forEach(typeName => {
    const hasType = fileContains(typeFile, `export interface ${typeName}`) || 
...
if (fileExists(forgeDetailsFile)) {
  ...
  stageComponents.forEach(component => {
    ...
    const isUsed = fileContains(forgeDetailsFile, `<${component}`) || 
                   fileContains(forgeDetailsFile, `{${component}}`);
    
    if (isImported && isUsed) {
      addResult('Integration', `${component} integrated`, 'PASS');
    } else if (isImported) {
      addResult('Integration', `${component} integrated`, 'WARN', 'Imported but not used');
    } else {
      addResult('Integration', `${component} integrated`, 'FAIL', 'Not imported or used');
    }
  });
} else {
  addResult('Integration', 'ForgeProjectDetails.tsx exists', 'FAIL', 'Missing integration file');
}
...
if (fileExists(e2eTestFile)) {
  ...
  addResult('E2E Tests', 'forge-workflow.spec.ts exists', 'FAIL', 'Missing E2E test file');
}
