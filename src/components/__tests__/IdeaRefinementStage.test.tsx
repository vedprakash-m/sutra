...
describe("IdeaRefinementStage", () => {
  ...
  it("moves to analysis section when Start Analysis is clicked", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Multi-Dimensional Analysis"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("handles technical complexity selection", async () => {
    ...
    const complexitySelect = screen.getByDisplayValue(
  ...
  it("calls onStageComplete when stage is completed", async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText("Complete Idea Refinement"),
        ).toBeInTheDocument();
      },
      { timeout: 3000 },
    );
    ...
    const problemTextarea = screen.getByPlaceholderText(
      /Clearly define the problem/,
    );
    const audienceTextarea = screen.getByPlaceholderText(
      /Describe who will use/,
    );
    const valueTextarea = screen.getByPlaceholderText(
