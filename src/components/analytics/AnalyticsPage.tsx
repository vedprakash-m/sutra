...
interface PerformanceMetrics {
  ...
  llmPerformance: Array<{ provider: string; avgTime: number; successRate: number }>;
  systemHealth: {
    cpuUsage: number;
    memoryUsage: number;
...
interface CostAnalytics {
  ...
  modelCosts: Record<string, { cost: number; requests: number; efficiency: number }>;
  projectedCosts: {
    thisMonth: number;
    nextMonth: number;
    trend: number;
...
const AnalyticsPage: React.FC = () => {
  ...
  const loadUsageAnalytics = async (): Promise<UsageAnalytics> => {
    if (user?.email) {
      try {
        const promptInsights = await analytics.analyzePromptPatterns(user.email);
    ...
    return {
      promptPatterns: [
        { pattern: 'Code Generation', frequency: 45, trend: 'increasing', efficiency: 85 },
        { pattern: 'Content Writing', frequency: 32, trend: 'stable', efficiency: 92 },
        { pattern: 'Data Analysis', frequency: 23, trend: 'decreasing', efficiency: 78 }
      ],
      userBehavior: [
        { action: 'Prompt Creation', count: 234, avgDuration: 45 },
        { action: 'Collection Management', count: 89, avgDuration: 120 },
  ...
  const loadCostAnalytics = async (): Promise<CostAnalytics> => {
    return {
      ...
      costOptimizations: [
        { suggestion: 'Switch to GPT-3.5-turbo for simple tasks', potentialSavings: 45.67, effort: 'Low' },
        { suggestion: 'Implement prompt caching for repeated queries', potentialSavings: 23.89, effort: 'Medium' },
        { suggestion: 'Optimize token usage in prompts', potentialSavings: 67.23, effort: 'High' }
      ]
    };
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Comprehensive insights and performance monitoring</p>
        </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-medium text-gray-900">Total Cost</h3>
                <span className="text-2xl">💰</span>
              </div>
              <div className="mt-2">
                <div className="text-2xl font-bold">${metrics.totalCost.toFixed(2)}</div>
                <p className={`text-xs ${metrics.costTrend < 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {metrics.costTrend > 0 ? '+' : ''}{metrics.costTrend.toFixed(1)}% from last period
                </p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="mt-2">
                ...
                <p className={`text-xs ${metrics.performanceTrend > 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {metrics.performanceTrend > 0 ? '+' : ''}{metrics.performanceTrend.toFixed(1)}% from last period
                </p>
              </div>
            </div>
              ...
              {Object.entries(metrics.modelDistribution).map(([model, percentage]) => (
                <div key={model} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 flex-1 ml-4">
                    ...
                    <span className="text-sm text-gray-600 min-w-12">{percentage}%</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeTab === 'usage' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Prompt Patterns */}
            <div className="bg-white p-6 rounded-lg shadow">
              <h3 className="text-lg font-medium text-gray-900 mb-4">📈 Prompt Patterns</h3>
              <div className="space-y-4">
                {usageData.promptPatterns.slice(0, 5).map((pattern, index) => (
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <span className={`px-2 py-1 rounded text-xs ${pattern.efficiency > 80 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="space-y-4">
                {Object.entries(usageData.featureUsage).map(([feature, percentage]) => (
          ...
          {/* User Behavior */}
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-lg font-medium text-gray-900 mb-4">👥 User Behavior Analytics</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {usageData.userBehavior.map((behavior, index) => (
                <div key={index} className="text-center p-4 border rounded-lg">
                  <p className="font-medium text-sm">{behavior.action}</p>
                  <p className="text-2xl font-bold text-blue-600">{behavior.count}</p>
                  <p className="text-xs text-gray-600">Avg: {behavior.avgDuration}s</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === 'performance' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {performanceData.apiResponseTimes.map((api, index) => (
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-xs text-gray-600">{api.avgTime}ms average</p>
                    </div>
                    <span className={`px-2 py-1 rounded text-xs ${api.trend < 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            ...
            {/* LLM Performance */}
            <div className="bg-white p-6 rounded-lg shadow">
              <h3 className="text-lg font-medium text-gray-900 mb-4">🤖 LLM Provider Performance</h3>
              <div className="space-y-4">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="text-center">
                ...
                <div className="text-2xl font-bold mb-1">{performanceData.systemHealth.cpuUsage}%</div>
              ...
              <div className="text-center">
                ...
                <div className="text-2xl font-bold mb-1">{performanceData.systemHealth.memoryUsage}%</div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
              ...
              <div className="text-center">
                ...
                <div className="text-2xl font-bold">{performanceData.systemHealth.activeConnections}</div>
              </div>
              <div className="text-center">
                <p className="text-sm font-medium mb-2">Uptime</p>
                <div className="text-2xl font-bold">{performanceData.systemHealth.uptime}%</div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
      ...
      {activeTab === 'costs' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              <h3 className="text-lg font-medium text-gray-900">Projected Next Month</h3>
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="text-3xl font-bold text-green-600">
                ${costData.costOptimizations.reduce((sum, opt) => sum + opt.potentialSavings, 0).toFixed(2)}
              </div>
              <p className="text-sm text-gray-600 mt-1">Available optimizations</p>
            </div>
          </div>
                ...
                <div key={model} className="flex items-center justify-between p-3 border rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {data.requests} requests | {data.efficiency.toFixed(1)}% efficiency
                    </p>
                  </div>
