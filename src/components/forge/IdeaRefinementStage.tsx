...
export default function IdeaRefinementStage({ 
  ...
  const [data, setData] = useState<IdeaRefinementData>({
    ...
    marketContext: {
      ...
      competitiveAdvantage: initialData?.marketContext?.competitiveAdvantage || ''
    },
    technicalFeasibility: {
  ...
  const [currentSection, setCurrentSection] = useState<'input' | 'analysis' | 'refinement'>('input');
  ...
  const analyzeIdeaQuality = useCallback(async (ideaData: IdeaRefinementData, context: any) => {
    try {
      const response = await fetch(`/api/forge/idea-refinement/analyze/${projectId}`, {
        method: 'POST',
        headers: {
  ...
  const refineIdeaWithLLM = useCallback(async (currentIdea: IdeaRefinementData, improvementFocus: string[], llmModel: string, context: any) => {
    try {
      const response = await fetch(`/api/forge/idea-refinement/refine/${projectId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const getQualityAssessment = useCallback(async () => {
    try {
      const response = await fetch(`/api/forge/idea-refinement/assessment/${projectId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        }
  ...
  const completeStage = useCallback(async (finalData: IdeaRefinementData, forceComplete: boolean = false) => {
    try {
      const response = await fetch(`/api/forge/idea-refinement/complete/${projectId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  }, [data.initialIdea, data.problemStatement, data.targetAudience, data.valueProposition]);
  ...
  const performMultiDimensionalAnalysis = async () => {
    ...
    setTimeout(() => {
      const mockAnalysis = {
        problem: {
          score: 75,
          insights: ["Problem is well-defined but could be more specific", "Clear pain point identified"],
          suggestions: ["Consider narrowing the scope to a specific user segment", "Define measurable success criteria"]
        },
        market: {
          score: 60,
          insights: ["Market size needs validation", "Competitive landscape partially understood"],
          suggestions: ["Research specific market segments", "Identify direct and indirect competitors"]
        },
        user: {
          score: 80,
          insights: ["Target audience clearly identified", "User needs well articulated"],
          suggestions: ["Create detailed user personas", "Validate assumptions with user interviews"]
        },
        technical: {
          score: 70,
          insights: ["Technical approach seems feasible", "Some complexity concerns identified"],
          suggestions: ["Break down technical requirements", "Identify potential technical risks early"]
        },
        competitive: {
          score: 65,
          insights: ["Some differentiation identified", "Competitive advantage could be stronger"],
          suggestions: ["Strengthen unique value proposition", "Identify barriers to entry"]
        }
      };
      ...
      const overallScore = Math.round(
        Object.values(mockAnalysis).reduce((sum, dimension: any) => sum + dimension.score, 0) / 5
      );
  ...
  const addCompetitor = () => {
    if (competitorInput.trim() && !data.marketContext.competitors.includes(competitorInput.trim())) {
      setData(prev => ({
        ...prev,
        marketContext: {
          ...prev.marketContext,
          competitors: [...prev.marketContext.competitors, competitorInput.trim()]
        }
      }));
      setCompetitorInput('');
    }
  ...
  const removeCompetitor = (competitor: string) => {
    setData(prev => ({
      ...
      marketContext: {
        ...prev.marketContext,
        competitors: prev.marketContext.competitors.filter(c => c !== competitor)
      }
    }));
  };

  const addConstraint = () => {
    if (constraintInput.trim() && !data.technicalFeasibility.constraints.includes(constraintInput.trim())) {
      setData(prev => ({
        ...prev,
        technicalFeasibility: {
          ...prev.technicalFeasibility,
          constraints: [...prev.technicalFeasibility.constraints, constraintInput.trim()]
        }
      }));
      setConstraintInput('');
    }
  ...
  const addRisk = () => {
    if (riskInput.trim() && !data.technicalFeasibility.risks.includes(riskInput.trim())) {
      setData(prev => ({
        ...prev,
        technicalFeasibility: {
          ...prev.technicalFeasibility,
  ...
  const handleLLMRefinement = async (improvementFocus: string[]) => {
    setIsRefining(true);
    try {
      const refinement = await refineIdeaWithLLM(data, improvementFocus, selectedLLM, projectContext);
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center">
        ...
        <p className="mt-2 text-lg text-gray-600">
          Transform your concept into a structured opportunity through systematic analysis
        </p>
        <div className="mt-4 text-sm text-gray-500">
          Using LLM: <span className="font-medium text-indigo-600">{selectedLLM}</span>
        </div>
      </div>

      {/* Progress Indicator */}
      <div className="flex items-center justify-center space-x-4">
        <div className={`flex items-center space-x-2 ${currentSection === 'input' ? 'text-indigo-600' : 'text-gray-400'}`}>
          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${currentSection === 'input' ? 'bg-indigo-100' : 'bg-gray-100'}`}>
            <LightBulbIcon className="h-4 w-4" />
          </div>
          <span className="text-sm font-medium">Input</span>
        </div>
        <ArrowRightIcon className="h-4 w-4 text-gray-400" />
        <div className={`flex items-center space-x-2 ${currentSection === 'analysis' ? 'text-indigo-600' : 'text-gray-400'}`}>
          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${currentSection === 'analysis' ? 'bg-indigo-100' : 'bg-gray-100'}`}>
            <ChartBarIcon className="h-4 w-4" />
          </div>
          <span className="text-sm font-medium">Analysis</span>
        </div>
        <ArrowRightIcon className="h-4 w-4 text-gray-400" />
        <div className={`flex items-center space-x-2 ${currentSection === 'refinement' ? 'text-indigo-600' : 'text-gray-400'}`}>
          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${currentSection === 'refinement' ? 'bg-indigo-100' : 'bg-gray-100'}`}>
      ...
      {currentSection === 'input' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Initial Idea</h2>
          <div className="space-y-4">
            <div>
              <label htmlFor="initialIdea" className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <textarea
                ...
                onChange={(e) => setData(prev => ({ ...prev, initialIdea: e.target.value }))}
      ...
      {currentSection === 'analysis' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="text-center">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Multi-Dimensional Analysis</h2>
            {isAnalyzing ? (
              <div className="py-12">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto mb-4"></div>
                <p className="text-gray-600">Analyzing your idea across multiple dimensions...</p>
                <div className="mt-4 space-y-2">
                  {ANALYSIS_DIMENSIONS.map((dimension, index) => (
                    <div key={dimension.id} className={`text-sm ${index <= 2 ? 'text-indigo-600' : 'text-gray-400'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {ANALYSIS_DIMENSIONS.map((dimension) => {
                  ...
                  return (
                    <div key={dimension.id} className="p-4 border border-gray-200 rounded-lg">
                      ...
                      {result && (
                        <>
                          <div className={`text-2xl font-bold mb-2 ${getQualityColor(result.score)}`}>
                            {result.score}%
                          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-gray-900">Overall Quality Score</h2>
            ...
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div 
                className={`h-3 rounded-full transition-all duration-300 ${
                  data.qualityScore >= 80 ? 'bg-green-500' : 
                  data.qualityScore >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                }`}
          ...
          {/* Refinement Form */}
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">Refine Your Idea</h2>
            ...
            <div className="space-y-6">
                ...
                {analysisResults.problem && (
                  <div className="mt-2 text-sm text-gray-600">
                    ...
                    <ul className="mt-1 list-disc list-inside">
                      {analysisResults.problem.suggestions.map((suggestion: string, index: number) => (
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setData(prev => ({
                        ...prev,
                        marketContext: { ...prev.marketContext, marketSize: e.target.value }
                      }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {data.marketContext.competitors.map((competitor, index) => (
                        <span
                          key={index}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setData(prev => ({
                        ...prev,
                        marketContext: { ...prev.marketContext, competitiveAdvantage: e.target.value }
                      }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
              ...
              <div>
                <h3 className="text-lg font-medium text-gray-900 mb-4">Technical Feasibility</h3>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setData(prev => ({
                        ...prev,
                        technicalFeasibility: { ...prev.technicalFeasibility, complexity: e.target.value as 'low' | 'medium' | 'high' }
                      }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                    >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {data.technicalFeasibility.constraints.map((constraint, index) => (
                        <span
                          key={index}
                        ...
                        >
                          ...
                          <button
                            onClick={() => setData(prev => ({
                              ...
                              technicalFeasibility: {
                                ...prev.technicalFeasibility,
                                constraints: prev.technicalFeasibility.constraints.filter((_, i) => i !== index)
                              }
                            }))}
                        ...
                        >
                          ...
                          <button
                            onClick={() => setData(prev => ({
                              ...
                              technicalFeasibility: {
                                ...prev.technicalFeasibility,
                                risks: prev.technicalFeasibility.risks.filter((_, i) => i !== index)
                              }
                            }))}
            ...
            <div className="mt-8 flex justify-end">
              <button
                ...
                disabled={!data.problemStatement || !data.targetAudience || !data.valueProposition}
