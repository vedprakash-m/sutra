...
import { CheckCircle, AlertTriangle, Clock, Download, Eye, FileText, Users, Target, Zap } from 'lucide-react';
...
export default function PRDGeneration({ 
  ...
  const [storyFormat, setStoryFormat] = useState<'standard' | 'gherkin' | 'invest'>('standard');
  const [prioritizationMethod, setPrioritizationMethod] = useState<'rice' | 'moscow' | 'kano'>('rice');
  ...
  const extractRequirements = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/forge/${projectId}/prd/extract-requirements`, {
        method: 'POST',
      ...
      if (result.costTracking) {
        trackCost(result.costTracking.cost, result.costTracking.model, 'forge_prd_requirements');
      }
      ...
      setValidationErrors(prev => [...prev, 'Requirements extraction failed']);
    } finally {
      setIsLoading(false);
    }
  }, [projectId, ideaRefinementData, requirementFocus, selectedLLM, trackCost, onQualityUpdate]);
  ...
  const generateUserStories = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/forge/${projectId}/prd/generate-user-stories`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      // Track costs
      if (result.costTracking) {
        trackCost(result.costTracking.cost, result.costTracking.model, 'forge_prd_stories');
      }
      ...
      toast.success(`Generated ${result.userStories.stories?.length || 0} user stories`);
  ...
  }, [projectId, prdData.requirements, ideaRefinementData, storyFormat, selectedLLM, trackCost]);
  ...
  const prioritizeFeatures = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/forge/${projectId}/prd/prioritize-features`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          features,
      ...
      // Track costs
      if (result.costTracking) {
        trackCost(result.costTracking.cost, result.costTracking.model, 'forge_prd_prioritization');
      }
      ...
      toast.success(`Prioritized ${result.prioritizedFeatures.features?.length || 0} features`);
  ...
  }, [projectId, prdData.userStories, prdData.requirements, ideaRefinementData, prioritizationMethod, selectedLLM, trackCost]);
  ...
  const generatePRDDocument = useCallback(async () => {
    ...
    try {
      const prdComponents = {
        ...
        nonFunctionalRequirements: prdData.requirements.nonFunctionalRequirements,
        businessRules: prdData.requirements.businessRules,
        assumptions: prdData.requirements.assumptions
      };

      const response = await fetch(`/api/forge/${projectId}/prd/generate-document`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prdComponents,
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to generate PRD document');
      setValidationErrors(prev => [...prev, 'PRD document generation failed']);
    } finally {
      setIsLoading(false);
    }
  ...
  const assessQuality = useCallback(async () => {
    ...
    try {
      ...
      const qualityMet = assessment.currentQuality.overallScore >= PRD_QUALITY_THRESHOLD;
      const consistencyOk = assessment.crossStageValidation.isConsistent;
      
      setCanProceed(qualityMet && consistencyOk);
      
      if (!consistencyOk) {
        setValidationErrors(assessment.crossStageValidation.validationErrors || []);
      }
  ...
  // Complete stage
  const completeStage = useCallback(async (forceComplete = false) => {
    ...
    if (!canProceed && !forceComplete) {
      toast.error(`Quality threshold not met (${overallQualityScore}% < ${PRD_QUALITY_THRESHOLD}%)`);
      return;
    }
    ...
    try {
      ...
      toast.success(`PRD stage completed with ${result.finalQualityScore}% quality`);
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to complete stage');
    } finally {
      setIsLoading(false);
    }
  }, [projectId, prdData, currentQuality, canProceed, overallQualityScore, onStageComplete]);

  // Effects
  useEffect(() => {
    if (prdData.requirements.functionalRequirements.length > 0 ||
    ...
    <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              {prdData.requirements.functionalRequirements.length > 0 && (
                <div className="space-y-4">
                  <h4 className="font-semibold">Functional Requirements ({prdData.requirements.functionalRequirements.length})</h4>
                  <div className="space-y-2">
                    {prdData.requirements.functionalRequirements.map((req) => (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            {expandedRequirement === req.id && (
                              <div className="space-y-2">
                                <div>
                                  <strong className="text-sm">Business Value:</strong>
                                  <p className="text-sm text-muted-foreground">{req.businessValue}</p>
                                </div>
          ...
          <Card>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button 
                    ...
                    disabled={isLoading || !prdData.requirements.functionalRequirements.length}
                    className="flex items-center gap-2"
                  >
                    {isLoading ? (
                      <>
            ...
            <CardContent className="space-y-4">
              {prdData.userStories.stories.length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold">User Stories ({prdData.userStories.stories.length})</h4>
                    <Badge variant="outline">
                      Avg INVEST: {prdData.userStories.stories.length > 0 ? 
                        (prdData.userStories.stories.reduce((sum, story) => sum + getInvestScore(story), 0) / prdData.userStories.stories.length).toFixed(1) : 0}%
                    </Badge>
                  </div>
                  <div className="space-y-2">
                    {prdData.userStories.stories.map((story) => (
                          ...
                          <div className="flex-1">
                            ...
                            {expandedStory === story.id && (
                              <div className="space-y-2">
                                <div>
                                  <strong className="text-sm">Business Value:</strong>
                                  <p className="text-sm text-muted-foreground">{story.businessValue}</p>
                                </div>
                                <div>
                                  <strong className="text-sm">Estimated Effort:</strong>
                                  <p className="text-sm text-muted-foreground">{story.estimatedEffort}</p>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-2 text-xs">
                                  {Object.entries(story.investValidation).map(([key, value]) => (
                                    <div key={key} className={`p-1 rounded text-center ${value ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}>
          ...
          <Card>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <select 
                    ...
                    onChange={(e) => setPrioritizationMethod(e.target.value as any)}
                    className="px-3 py-1 border rounded text-sm"
                  >
            ...
            <CardContent className="space-y-4">
              {prdData.prioritizedFeatures.features.length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold">Prioritized Features ({prdData.prioritizedFeatures.features.length})</h4>
                    <Badge variant="outline">
                      MVP Candidates: {prdData.prioritizedFeatures.features.filter(f => f.mvpCandidate).length}
                    </Badge>
                  </div>
                  <div className="space-y-2">
                    {prdData.prioritizedFeatures.features
                      ...
                      .map((feature) => (
                      <Card key={feature.featureId} className={`p-4 ${feature.mvpCandidate ? 'border-primary' : ''}`}>
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <Badge variant="outline">{feature.featureId}</Badge>
                            ...
                            <h5 className="font-medium mb-1">{feature.featureName}</h5>
                            <p className="text-sm text-muted-foreground mb-2">
                              {feature.description}
                            </p>
                                ...
                                <div className="flex items-center gap-1">
                                  <div className="w-16 bg-gray-200 rounded-full h-2">
                                    <div 
                                      ...
                                      style={{ width: `${feature.businessValue * 10}%` }}
                                    />
                                  </div>
                                ...
                                <div className="flex items-center gap-1">
                                  <div className="w-16 bg-gray-200 rounded-full h-2">
                                    <div 
                                      ...
                                      style={{ width: `${feature.userValue * 10}%` }}
                                    />
                                  </div>
                                ...
                                <div className="flex items-center gap-1">
                                  <div className="w-16 bg-gray-200 rounded-full h-2">
                                    <div 
                                      ...
                                      style={{ width: `${feature.alignmentScore * 10}%` }}
                                    />
                                  </div>
                            ...
                            <p className="text-xs text-muted-foreground mt-2">{feature.rationale}</p>
                          </div>
                        </div>
                      </Card>
                    ))}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button 
                    ...
                    disabled={isLoading || !prdData.prioritizedFeatures.features.length}
                    className="flex items-center gap-2"
                  >
                    {isLoading ? (
                      <>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div className="text-center p-3 bg-gray-50 rounded">
                      <div className="font-semibold">{prdData.prdDocument.documentMetrics?.totalSections || 0}</div>
                      <div className="text-muted-foreground">Sections</div>
                    </div>
                    <div className="text-center p-3 bg-gray-50 rounded">
                      <div className="font-semibold">{prdData.prdDocument.documentMetrics?.wordCount || 0}</div>
                      <div className="text-muted-foreground">Words</div>
                    </div>
                    <div className="text-center p-3 bg-gray-50 rounded">
                      <div className="font-semibold">{prdData.prdDocument.documentMetrics?.completedSections || 0}</div>
                      <div className="text-muted-foreground">Complete</div>
                    </div>
                    <div className="text-center p-3 bg-gray-50 rounded">
                      <div className="font-semibold">{prdData.prdDocument.documentMetrics?.readabilityScore?.toFixed(0) || 0}%</div>
                      <div className="text-muted-foreground">Readability</div>
                    </div>
                  </div>
                  
                  {showDocumentPreview && prdData.prdDocument.exportData?.content && (
      ...
      <div className="flex items-center justify-between pt-6 border-t">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {progressPercentage === 100 ? 'Stage Complete' : `${progressPercentage.toFixed(0)}% Complete`}
        </div>
        
        <div className="flex items-center gap-2">
          {!canProceed && overallQualityScore < PRD_QUALITY_THRESHOLD && user?.role === 'expert' && (
            <Button
              variant="outline"
