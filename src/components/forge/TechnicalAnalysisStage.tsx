...
const TechnicalAnalysisStage: React.FC<TechnicalAnalysisStageProps> = ({
  ...
  const [currentStep, setCurrentStep] = useState<
    "setup" | "evaluation" | "review" | "export"
  >("setup");
  ...
  // Results state
  const [technicalEvaluation, setTechnicalEvaluation] =
    useState<TechnicalEvaluation | null>(null);
  ...
  const estimatedCost = useMemo(() => {
    ...
    const avgCost =
      selectedModelData.reduce(
        (sum, model) => sum + model.cost_per_1k_tokens,
        0,
  ...
  const handleStartEvaluation = async () => {
    if (!canProceedToEvaluation) {
      toast({
        ...
        description:
          "Please select at least 2 models and ensure all previous stages are complete",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleRetryEvaluation = async () => {
    ...
    if (availableForRetry.length > 0) {
      ...
      toast({
        ...
        description:
          "All available models have been used. Consider adjusting parameters instead.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const content =
          exportFormat === "markdown"
            ...
            : JSON.stringify(result.exported_content, null, 2);
      ...
      toast({
        title: "Export Failed",
        description:
          error instanceof Error ? error.message : "Failed to export analysis",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleComplete = () => {
    if (technicalEvaluation && qualityMetrics?.meets_threshold) {
      ...
      toast({
        ...
        description:
          "Please retry evaluation or adjust parameters to meet the 85% quality threshold",
        variant: "destructive",
      });
    }
  };
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleModelSelectionChange(model.id, checked as boolean)
                    }
                  />
                  <div className="flex-1 space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {model.provider} • ${model.cost_per_1k_tokens.toFixed(3)}
                      /1K tokens
                    </p>
          ...
          <div className="space-y-3">
            ...
            {!canProceedToEvaluation && (
              <Alert>
                ...
                <AlertDescription>
                  Complete previous stages and select at least 2 models to
                  proceed
                </AlertDescription>
              </Alert>
            )}
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <RefreshCw className="h-4 w-4 animate-spin" />
              <span>
                Analyzing architecture across {selectedModels.length} LLM
                models...
              </span>
            </div>
            ...
            <p className="text-sm text-muted-foreground">
              This may take 30-60 seconds depending on the number of models
              selected. Please wait while we generate comprehensive technical
              recommendations.
            </p>
          </div>
        </CardContent>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center space-y-2">
                ...
                <Badge
                  variant={
                    consensusResult.consensus_level === "strong_agreement"
                      ? "default"
                      : consensusResult.consensus_level === "moderate_agreement"
                        ? "secondary"
                        : "destructive"
                  }
                  className="mt-1"
                >
                  {consensusResult.consensus_level
                    .replace("_", " ")
                    .toUpperCase()}
                </Badge>
              </div>
            ...
            {!qualityMetrics.meets_threshold && (
              <Alert className="mt-4">
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  Quality score is below the {qualityMetrics.threshold}%
                  threshold. Consider retrying with additional models or
                  reviewing consensus conflicts.
                </AlertDescription>
              </Alert>
            )}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                  ...
                  <div className="space-y-3">
                    <div>
                      <Badge variant="default" className="text-sm">
                        {
                          technicalEvaluation.architecture_recommendation
                            .pattern
                        }
                      </Badge>
                      <div className="mt-2 text-sm text-muted-foreground">
                        Consensus:{" "}
                        {technicalEvaluation.architecture_recommendation.votes}/
                        {
                          technicalEvaluation.architecture_recommendation
                            .total_responses
                        }{" "}
              ...
              <TabsContent value="technology" className="space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(technicalEvaluation.technology_stack).map(
                      ([category, tech]) => (
                        <Card key={category}>
                          ...
                          <CardContent>
                            <div className="space-y-2">
                              ...
                              <div className="text-xs text-muted-foreground">
                                {tech.vote_count}/
                                {consensusResult.models_used.length} models (
                                {(tech.consensus_strength * 100).toFixed(1)}%
                                consensus)
                              </div>
              ...
              <TabsContent value="feasibility" className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">
                        {technicalEvaluation.feasibility_assessment.overall_feasibility_score.toFixed(
                          1,
                        )}
                        /10
                      </div>
                      <Progress
                        value={
                          technicalEvaluation.feasibility_assessment
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      <ul className="space-y-1">
                        {technicalEvaluation.feasibility_assessment.key_challenges.map(
                          (challenge, idx) => (
                            <li
                              key={idx}
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <ul className="space-y-1">
                        {technicalEvaluation.feasibility_assessment.success_factors.map(
                          (factor, idx) => (
                            <li
                              key={idx}
                              className="text-sm flex items-start gap-2"
              ...
              <TabsContent value="risks" className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">
                        {technicalEvaluation.risk_analysis.overall_risk_level.toFixed(
                          1,
                        )}
                        /10
                      </div>
                      <Progress
                        value={
                          technicalEvaluation.risk_analysis.overall_risk_level *
                          10
                        }
                        className="mt-2"
                      />
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        {technicalEvaluation.risk_analysis.top_risks
                          .slice(0, 3)
                          .map((risk, idx) => (
                            <div
                              key={idx}
                ...
                <div className="space-y-3">
                  {Object.entries(
                    technicalEvaluation.risk_analysis.risk_categories,
                  ).map(([category, riskData]) => (
                    <Card key={category}>
                      <CardHeader className="pb-2">
              ...
              <TabsContent value="roadmap" className="space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">
                          {
                            technicalEvaluation.implementation_roadmap
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">
                          {
                            technicalEvaluation.implementation_roadmap
                              .team_requirements.recommended_size
                          }{" "}
                          people
                        </div>
                      </CardContent>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">
                          {
                            technicalEvaluation.implementation_roadmap.phases
                              .length
                          }
                        </div>
                      </CardContent>
                  ...
                  <div className="space-y-4">
                    {technicalEvaluation.implementation_roadmap.phases.map(
                      (phase, idx) => (
                        <Card key={idx}>
                          <CardHeader className="pb-2">
                  ...
                  <Card className="mt-6">
                    ...
                    <CardContent>
                      <div className="flex flex-wrap gap-2">
                        {technicalEvaluation.implementation_roadmap.team_requirements.key_roles.map(
                          (role) => (
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value: string) =>
                setExportFormat(value as "markdown" | "json")
              }
            >
              <SelectTrigger>
                <SelectValue />
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">
          Multi-LLM consensus evaluation for architecture, feasibility, and
              ...
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium ${
                  step === currentStep
                    ...
                    : ["setup", "evaluation", "review"].indexOf(currentStep) >
                        index
