  ...
  const handleTestPrompt = async () => {
    ...
    const promises = selectedLLMs.map(async (provider) => {
      try {
        ...
        outputs[provider] = {
          provider,
          response: response?.data || response?.response || 'No response received',
          loading: false
        }
      } catch (error) {
        outputs[provider] = {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
          <div className="bg-white shadow rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4">Prompt Details</h2>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4">Prompt Text</h2>
            <div>
              <textarea
            ...
            <div className="space-y-2">
              {['openai', 'anthropic', 'google'].map((llm) => (
                <label key={llm} className="flex items-center">
                  ...
                  />
                  <span className="ml-2 text-sm text-gray-700 capitalize">{llm}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="flex space-x-4">
            ...
            <button
              ...
              disabled={isSaving || !promptData.title.trim() || !promptData.content.trim()}
        ...
        <div className="space-y-6">
          <div className="bg-white shadow rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4">LLM Outputs</h2>
            <div className="space-y-4">
              {selectedLLMs.map((llm) => {
                const output = llmOutputs[llm]
                return (
                  <div key={llm} className="border border-gray-200 rounded-lg p-4">
          ...
          <div className="bg-white shadow rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4">Prompt Coach</h2>
            <div className="bg-blue-50 rounded-lg p-4">
              <p className="text-sm text-blue-700">
                💡 AI-powered suggestions will appear here to help improve your prompt
              </p>
            </div>
          </div>
        </div>
