...
jest.mock("../PromptCoach", () => {
  return function MockPromptCoach() {
    return (
      <div data-testid="prompt-coach">
        Prompt Coach Component
      </div>
    );
  };
});
...
beforeEach(() => {
  ...
  (collectionsApi.create as jest.Mock).mockResolvedValue({ id: "test-collection-id" });
...
describe("PromptBuilder", () => {
  it("should render the prompt builder with title", () => {
    ...
    expect(screen.getByText("Create and test AI prompts with multi-LLM comparison")).toBeInTheDocument();
  });
  ...
  it("should handle content input changes", () => {
    ...
    const contentTextarea = screen.getByPlaceholderText("Write your prompt here...");
    fireEvent.change(contentTextarea, { target: { value: "Write a story about {topic}" } });
  ...
  it("should test prompt with selected LLMs", async () => {
    ...
    const contentTextarea = screen.getByPlaceholderText("Write your prompt here...");
    fireEvent.change(contentTextarea, { target: { value: "Write a story about cats" } });
  ...
  it("should display loading state during testing", async () => {
    (llmApi.execute as jest.Mock).mockImplementation(() => new Promise(() => {}));
  ...
  it("should handle variables in prompt", () => {
    ...
    const contentTextarea = screen.getByPlaceholderText("Write your prompt here...");
    fireEvent.change(contentTextarea, { target: { value: "Write about {topic} in {style}" } });
