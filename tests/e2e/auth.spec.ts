...
test.describe("Authentication and User Management", () => {
  ...
  test("should display login page correctly", async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Sign in (Development Mode)")'),
    ).toBeVisible();
  });

  test("should allow regular user login and access dashboard", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('nav a:has-text("Prompt Builder")'),
    ).toBeVisible();
  ...
  test("should allow admin user login and access admin panel", async ({
    page,
  }) => {
  ...
  test("should maintain user session across page navigation", async ({
    page,
  }) => {
    await helpers.loginAsUser("user");
