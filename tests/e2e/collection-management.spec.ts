...
test.describe("Collection Management", () => {
  ...
  test("should display collections page correctly", async ({ page }) => {
    ...
    await expect(
      page.locator(
        'button:has-text("Create Collection"), button:has-text("New Collection")',
      ),
    ).toBeVisible();
  ...
  test("should search collections", async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      const collectionItems = page.locator(
        '[data-testid="collection-item"], .collection-item',
      );
  ...
  test("should view collection details", async ({ page }) => {
    ...
    const collectionLink = page
      .locator('a[href*="/collections/"], button:has-text("View Details")')
      .first();
  ...
  test("should add prompt to collection", async ({ page }) => {
    ...
    await helpers.createCollection(
      collectionName,
      "Collection for prompt testing",
    );
  ...
  test("should filter collections by type", async ({ page }) => {
    ...
    if (await filterButton.isVisible()) {
      ...
      const privateFilter = page.locator(
        'button:has-text("Private"), option[value="private"]',
      );
  ...
  test("should import prompts to collection", async ({ page }) => {
    ...
    const importButton = page.locator(
      'button:has-text("Import"), button:has-text("Import Prompts")',
    );
  ...
  test("should manage collection permissions", async ({ page }) => {
    ...
    const settingsButton = page
      .locator(
        'button:has-text("Settings"), button:has-text("Share"), [data-testid="collection-settings"]',
      )
      .first();
    ...
    if (await settingsButton.isVisible()) {
      ...
      const permissionOptions = page.locator(
        "text=Private, text=Shared, text=Public",
      );
