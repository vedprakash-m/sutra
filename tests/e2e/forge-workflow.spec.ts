...
test.describe("Forge Workflow - Complete 5-Stage Journey", () => {
  ...
  test("should complete full Forge workflow from Stage 1 to Stage 5", async ({
    ...
    await expect(
      page.locator('[data-stage="idea_refinement"]')
    ...
    for (let i = 0; i < Math.min(questionCount, 3); i++) {
      await questionInputs.nth(i).fill(
    ...
    // Check quality score
    const qualityScore = await page.locator('[data-quality-score]').textContent();
    ...
    await expect(
      page.locator('[data-stage="prd_generation"]')
    ...
    const prdQualityScore = await page.locator('[data-quality-score]').textContent();
    ...
    await expect(
      page.locator('[data-stage="ux_requirements"]')
    ).toHaveClass(/active|current/, { timeout: 10000 });
    ...
    const uxQualityScore = await page.locator('[data-quality-score]').textContent();
    const accessibilityScore = await page.locator('[data-accessibility-score]').textContent();
    console.log(`UX Quality Score: ${uxQualityScore}, Accessibility: ${accessibilityScore}`);
    ...
    await expect(
      page.locator('[data-stage="technical_analysis"]')
    ).toHaveClass(/active|current/, { timeout: 10000 });
    ...
    const consensusScore = await page.locator('[data-consensus-score]').textContent();
    ...
    await expect(page.locator('[data-stage="idea_refinement"][data-status="completed"]')).toBeVisible();
    await expect(page.locator('[data-stage="prd_generation"][data-status="completed"]')).toBeVisible();
    await expect(page.locator('[data-stage="ux_requirements"][data-status="completed"]')).toBeVisible();
    await expect(page.locator('[data-stage="technical_analysis"][data-status="completed"]')).toBeVisible();
    await expect(page.locator('[data-stage="implementation_playbook"][data-status="completed"]')).toBeVisible();
    ...
    await expect(
      page.locator(`text=${projectName}`).locator("..").locator('text=/Completed/i')
    ).toBeVisible();
  ...
  test("should enforce quality gates between stages", async ({ page }) => {
    ...
    await expect(
      page.locator('text=/Quality threshold not met/i')
  ...
  test("should preserve context across stage transitions", async ({ page }) => {
    ...
    const testIdea = "Context Preservation Test - AI-powered analytics dashboard";
    ...
    const stage1Insights = await page.locator('[data-key-insights]').textContent();
    console.log(`Stage 1 Insights captured: ${stage1Insights?.substring(0, 50)}...`);
  ...
  test("should handle navigation between stages correctly", async ({ page }) => {
    ...
    console.log("✅ Successfully navigated to Stage 2 after completing Stage 1");
  });
});
...
test.describe("Forge Workflow - Cost Tracking", () => {
  ...
  test("should track costs across all Forge stages", async ({ page }) => {
    ...
    await expect(page.locator('[data-stage-cost="idea_refinement"]')).toBeVisible();
  });
});
