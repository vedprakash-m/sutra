...
async function initializeTestData() {
  ...
  try {
    ...
    try {
      ...
      const resetResponse = await page.request.post('http://localhost:7071/api/admin/test-data/reset', {
        headers: {
      ...
      if (resetResponse.ok()) {
        ...
        console.log('✅ Test data cleared successfully:', resetData.containers_reset?.length || 0, 'containers')
      } else {
        console.log('⚠️ Could not clear existing data, status:', resetResponse.status())
      }
    } catch (error) {
    ...
    try {
      ...
      const seedResponse = await page.request.post('http://localhost:7071/api/admin/test-data/seed', {
        data: {
          includeUsers: true,
      ...
      if (seedResponse.ok()) {
        ...
        console.log('✅ Test data seeded successfully:', seedData.data_created?.length || 0, 'items created')
      } else {
        console.log('⚠️ Could not seed test data, status:', seedResponse.status())
