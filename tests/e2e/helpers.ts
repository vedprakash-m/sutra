...
export class TestHelpers {
  ...
  async loginAsUser(userType: 'user' | 'admin' = 'user') {
    ...
    await expect(this.page.locator('h1').filter({ hasText: 'Welcome back' })).toBeVisible({ timeout: 10000 })
  ...
  async createPrompt(title: string, content: string, description?: string) {
    ...
    if (description) {
      await this.page.fill('textarea[placeholder="Describe what this prompt does"]', description)
    }
    await this.page.fill('textarea[placeholder="Write your prompt here..."]', content)
  ...
  async createPlaybook(name: string, description: string) {
    ...
    await this.page.fill('textarea[placeholder="Describe what this playbook does"]', description)
    ...
    // Fill step content
    await this.page.fill('textarea[placeholder="Enter step content..."]', 'This is a test step')
  ...
  async expectNoErrors() {
    const errorElements = this.page.locator('[role="alert"], .error, .text-red-500')
  ...
  async clearTestData() {
    try {
      await this.page.request.post('http://localhost:7071/api/admin/reset-test-data', {
        headers: {
