...
test.describe("Playbook Management", () => {
  ...
  test("should display playbook builder correctly", async ({ page }) => {
    ...
    // Check form elements
    await expect(
      page.locator('input[placeholder="Enter playbook name"]'),
    ).toBeVisible();
    await expect(
      page.locator('textarea[placeholder="Describe what this playbook does"]'),
    ).toBeVisible();
  ...
  test("should add different types of steps", async ({ page }) => {
    ...
    await helpers.fillField(
      ...
      `Multi-Step Playbook ${Date.now()}`,
    );
    await helpers.fillField(
      'textarea[placeholder="Describe what this playbook does"]',
      "Testing different step types",
    );

    // Add prompt step
    await page.click('button:has-text("Add Step")');
    const promptStepOption = page.locator(
      'button:has-text("Prompt Step"), text=Prompt',
    );
    if (await promptStepOption.isVisible()) {
      ...
      const stepContent = page
        .locator('textarea[placeholder="Enter step content..."], textarea')
        .last();
    ...
    await page.click('button:has-text("Add Step")');
    const reviewStepOption = page.locator(
      'button:has-text("Review Step"), text=Review',
    );
    ...
    await page.click('button:has-text("Add Step")');
    const variableStepOption = page.locator(
      'button:has-text("Variable Step"), text=Variable',
    );
  ...
  test("should reorder playbook steps", async ({ page }) => {
    ...
    await helpers.fillField(
      'input[placeholder="Enter playbook name"]',
      `Reorder Test ${Date.now()}`,
    );
    await helpers.fillField(
      'textarea[placeholder="Describe what this playbook does"]',
      "Testing step reordering",
    );
    ...
    const moveUpButton = page.locator(
      'button[title="Move up"], button:has-text("↑")',
    );
  ...
  test("should execute playbook with PlaybookRunner", async ({ page }) => {
    ...
    if (await existingPlaybook.isVisible()) {
      ...
      const runnerInterface = page.locator(
        "text=Playbook Runner, text=Execute Playbook",
      );
      if (await runnerInterface.isVisible()) {
        ...
        if (await startButton.isVisible()) {
          ...
          const statusIndicator = page.locator(
            "text=Running, text=Executing, text=Complete",
          );
      ...
      await helpers.createPlaybook(
        `Execution Test ${Date.now()}`,
        "Playbook for execution testing",
      );
    }
  ...
  test("should handle playbook variables", async ({ page }) => {
    ...
    // Add variable step
    await page.click('button:has-text("Add Step")');
    const variableOption = page.locator(
      'button:has-text("Variable"), text=Variable',
    );
    if (await variableOption.isVisible()) {
      await variableOption.click();

      // Define a variable
      const variableInput = page.locator(
        'input[placeholder*="variable"], input[name*="variable"]',
      );
      if (await variableInput.isVisible()) {
        ...
        const valueInput = page.locator(
          'input[placeholder*="value"], input[name*="value"]',
        );
