...
test.describe("Prompt Management", () => {
  ...
  test("should display prompt builder page correctly", async ({ page }) => {
    ...
    // Check form elements
    await expect(
      page.locator('input[placeholder="Enter prompt title"]'),
    ).toBeVisible();
    await expect(
      page.locator('textarea[placeholder="Describe what this prompt does"]'),
    ).toBeVisible();
    await expect(
      page.locator('textarea[placeholder="Write your prompt here..."]'),
    ).toBeVisible();
  ...
  test("should provide prompt suggestions with PromptCoach", async ({
    page,
  }) => {
    await helpers.navigateTo("Prompt Builder");
